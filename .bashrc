...
cv() {
    if ! local dir="$(findup -d vendor/phibrenet)"; then
        echo 'No vendor/phibrenet/ directory found' >&2
        return 1
    fi

    if [[ -z $1 ]]; then
        c "$dir/vendor/phibrenet"
    elif [[ -d "$dir/vendor/phibrenet/$1" ]]; then
        c "$dir/vendor/phibrenet/$1"
    else
        local matches=("$dir/vendor/phibrenet/"*"$1"*)
